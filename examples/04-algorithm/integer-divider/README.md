# IntegerDivider — RISC-V 整数除法器

> UT 快速入门见 [01-baseline/adder](../../01-baseline/adder/)。本例为 **04-algorithm flagship**。

## 验证故事

验证 Radix-4 SRT 除法器在多模块 Chisel 实现下的功能正确性与 corner case 覆盖。
...
- `picker`、Python 3.11+
- 需阅读 `*.scala` 与 `IntegerDivider.v` 做 bug 分析

## 监督式 Codex 运行

| 项 | 值 |
|----|-----|
| Workflow | `examples/04-algorithm/workflow/default.yaml` |

```bash
make example-algorithm
# 或：make mcp_IntegerDivider
```
...
- 有/无符号除法路径与符号修正
- 除零、溢出、`io_flush` 中止
- PreProcess 归一化与迭代周期计数
- DivStage 商位选择 {-2,-1,0,1,2}

## 预期产物

- 完整 11-stage UT 文档与 pytest 套件
- 不预置 golden
...
- [Examples 索引](../../README.md) · [04-algorithm 索引](../README.md)

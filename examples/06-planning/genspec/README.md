...
> 属于 [06-planning](../README.md) 故事线。DCache 为 **advanced** 例；入门可用同目录 `Adder/`。

## 验证故事

把碎片化设计资料与 RTL 整合为结构化 `{DUT}_spec.md` 与 FG/FC/CK 文档。

## 难度

| DUT | 级别 |
|-----|------|
| Adder | 入门（同 01-baseline DUT，Spec-only workflow） |
| DCache | **Advanced**（体量大，建议跳过人工 stage 做自动化演示） |
| Sbuffer | Advanced |
...
make init_DCache
make spec_DCache
# 或：make spec_mcp_DCache（同 spec_DCache）
```
...
GenSpec 产出 spec 与 functions_and_checks，是 UT workflow 的**上游**；完成后可接 01-baseline 式全量验证。

---

Spec 文档是进行芯片验证与回归管理的基础。很多团队在项目早期只有零散的设计备忘、接口列表或旧版论文，缺乏结构化、可复用的规格说明。GenSpec 示例展示了如何借助 VeriAgent 的自定义配置把这些碎片化资料整合成系统化的 `{DUT}_spec.md`，并在持续迭代中保持更新。
...
# 以API方式运行veriagent生成spec文档，如果不需要自动开始，请去掉 ARGS="--loop"
make spec_DCache ARGS="--loop"

# 或者通初始化veriagent命令取消人工检查和最后一个阶段

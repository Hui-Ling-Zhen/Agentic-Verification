...
VeriAgent 采用 **YAML 作为唯一事实来源 (SSOT)** 的架构。
...
```bash
python3 .veriagent/skills/formal/sva-gen/scripts/update_sva_body.py <CK-ID> "<SVA_CODE_BODY>"
```

**示例：**
```bash
python3 .veriagent/skills/formal/sva-gen/scripts/update_sva_body.py CK-ADD-CORE-EQUATION "##0 ({cout, sum} == a + b + cin);"
```

### 3. 调用 Check 工具验证

调用 `veriagent.checkers.formal.PropertyStructureChecker`。
- 如果仍有检测点未填写，Checker 会报错并列出缺失项。
- 如果全部填写完成，Checker 会自动渲染生成最新的 `checker.sv` 和 `wrapper.sv` 供后续 EDA 工具使用。

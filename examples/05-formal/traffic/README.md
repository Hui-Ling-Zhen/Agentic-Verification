# Formal traffic — 交通灯控制器（进阶）

## 验证故事

多模块状态机（农场/高速/定时器）的形式化验证，比 arbiter 更复杂。

## 难度

预计 Agent 轮次：很高 | Mock：否 | Formal：**是** | 需 `--use-skill`

## 快速运行

```bash
make formal_mcp_traffic
```

```bash
veriagent examples/05-formal/output/workspace_traffic/ traffic \
...
- 多 FSM 协同与定时约束
- 覆盖 farm / hwy / timer 子模块属性
- 反例分析与 Python 回归测试

## 预期产物

- `formal_test/` 下完整 Formal 流水线产物

## 延伸阅读

- [Formal arbiter](../arbiter/README.md)（建议先跑） · [05-formal 索引](../README.md)

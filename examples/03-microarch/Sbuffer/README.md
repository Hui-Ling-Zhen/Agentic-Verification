...
**03-microarch flagship**：Mock 环境 + 黑盒验证代表例。

## 快速运行

```bash
make example-flagship                    # 仓库根目录（Mock + BBV）
cd examples/03-microarch/Sbuffer && make run-with-mock
```

## 验证关注点

- Store buffer 入队/出队与 DCache 交互
- Forward 查询与流水线宽度 corner case
- Mock 组件与黑盒（BBV）模式

## 预期产物

- 完整 11-stage UT 文档与 pytest（含 Mock 相关 stage）

## 延伸阅读

- [Examples 索引](../../README.md) · [03-microarch 索引](../README.md)
...
### 2.2 验证计划结构
为了便于管理，请按照 VeriAgent 标签规范组织测试点。详细功能点与检测点可参考 `Sbuffer_functions_and_checks.md`。
...
发现 bug 按照 VeriAgent 的 bug 记录规范进行记录。
...
| 项 | 值 |
|----|-----|
| Workflow | `examples/03-microarch/workflow/default.yaml` |

```bash
make run-with-mock          # Mock + BBV（推荐）
make example-flagship       # 仓库根目录
```
...
不启用 Mock 三阶段，适合已有完整上下游环境或先熟悉 DUT spec 时。

```bash
make run
# 或仓库根目录：make mcp_Sbuffer
```

### 5.2 Mock + 黑盒模式（推荐 flagship）

Sbuffer 无法独立仿真，需 Mock 上下游总线。启用后 workflow 增加 3 个 Stage：

1. **mock_design_and_implementation** — 设计 Mock 组件（`{OUT}/tests/{DUT}_mock_*.py`）
2. **mock_fixture_implementation** — 注册 `mock_dut` fixture
3. **mock_functional_test** — Mock 组件自测

```bash
make run-with-mock
# 或仓库根目录：make example-flagship
```
...
| 变量 | 普通 | Mock 模式 |
|------|------|-----------|
| `IGNORE_MOCK_COMPONENT` | `true`（默认） | `false` |
| `BBV` | `false` | `true`（清空 RTL，黑盒验证） |
| `ENABLE_LLM_FAIL_SUGGESTION` | 可选 | 建议 `true` |
| `ENABLE_LLM_PASS_SUGGESTION` | 可选 | 建议 `true` |

Mock + BBV 一键命令（已含监督式 Codex 参数）：
...
- Mock 应尽量简单，只模拟必要握手与数据通路
- 通过 `on_clock_edge` 驱动 DUT 信号
- **必须先通过 Mock 功能测试**，再测 DUT 本体
- 复杂协议建议人机协同：Agent 搭框架，人工补关键时序

### 5.4 预期 Mock 产物

- `{OUT}/tests/Sbuffer_mock_*.py` — Mock 实现
- `{OUT}/tests/Sbuffer_api.py` — 含 `mock_dut` fixture
- `{OUT}/tests/test_Sbuffer_mock_*.py` — Mock 自测

## 验证故事

用处理器 Store Buffer 证明 Agent 能在 **Mock 环境 + 黑盒 RTL** 下完成微架构级功能验证。

## 难度

预计 Agent 轮次：很高 | Mock：**是** | Formal：否 | 人工审查：建议
...
- Store 入队/合并与 multi-port 入队
- Load forwarding（vtag/ptag、matchInvalid）
- DCache 写回与 PLRU 替换
- Flush / drain 状态机

## 与 Agentic Verification 的关系

**Flagship 难例**：代表 Agentic Verification 在真实 CPU 微架构模块上的能力上限。

## 延伸阅读

- [03-microarch 总览](../README.md)

...
- 配置字段：
  ...
  - `clss`: 检查器类名；短名默认从 `veriagent.checkers` 命名空间导入，也可写完整模块路径（如 `mypkg.mychk.MyChecker`）
...
（可扩展）自定义检查器（最小实现，放在 `veriagent/checkers/unity_test.py`）

很多场景下“增加的 checker”并非复用已有检查器，而是需要自己实现一个新的检查器。最小实现步骤：

1. 新建类并继承基类 `veriagent.checkers.base.Checker`
...
5. 若想用短名 `clss` 引用，请在 `veriagent/checkers/__init__.py` 导出该类（或在 `clss` 写完整模块路径）

最小代码骨架（示例）：

```python
# 文件：veriagent/checkers/unity_test.py
...
- 在 `veriagent/checkers/` 新增类，继承 `veriagent.checkers.base.Checker` 并实现 `do_check()`；
- 在 `veriagent/checkers/__init__.py` 导出类后，可在 `clss` 用短名；或直接写完整模块路径；
- `args` 中的字符串支持模板变量渲染；`extra_args` 可用于自定义提示文案（具体视检查器实现而定）。

### 常用 checker 参数（结构化）

以下参数均来自实际代码实现（`veriagent/checkers/unity_test.py`），名称、默认值与类型与代码保持一致；示例片段可直接放入阶段 YAML 的 `checker[].args`。
...
- [架构与工作原理](02_architecture.md) - 了解 VeriAgent 的核心架构

...
在开始定制自己的工作流之前，了解 VeriAgent 的整体架构和工作原理至关重要。本章将为您揭示 VeriAgent 的内部运作机制，帮助您建立全局认知。

## VeriAgent 是什么？

VeriAgent 是一个基于大语言模型（LLM）的智能验证代理系统。它通过**工作流（Workflow）**定义验证任务，利用**工具（Tools）**执行操作，通过**检查器（Checkers）**验证质量，参考**模板（Templates）**保证规范，最终实现自动化的验证流程。

## 核心架构

VeriAgent 由三大核心组件构成，它们相互协作完成验证任务：
...
现在让我们通过**计算器文档生成器**这个实际场景，详细了解 VeriAgent 的完整执行流程：

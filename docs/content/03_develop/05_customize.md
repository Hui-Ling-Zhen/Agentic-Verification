...
- `veriagent/tools/uctool.py`：工具基类 UCTool、to_fastmcp（LangChain Tool → FastMCP Tool）
- `veriagent/util/functions.py`：`import_and_instance_tools`（按名称导入实例）、`create_verify_mcps`（启动 FastMCP）
- `veriagent/verify_agent.py`：装配工具清单，`start_mcps` 组合并启动 Server
- `veriagent/cli.py` / `veriagent/verify_pdb.py`：命令行与 TUI 内的 MCP 启动命令
...
- 名称解析：
  - 短名：类/工厂函数需在 `veriagent/tools/__init__.py` 导出（例如 `from .mytool import HelloTool`），即可在 `ex_tools` 写 `HelloTool`
...
from veriagent.tools.uctool import UCTool
...
（可选）短名注册：在 `veriagent/tools/__init__.py` 导出 `HelloTool` 后，可写 `--ex-tools HelloTool`。
...
工具 → MCP 转换（`veriagent/tools/uctool.py::to_fastmcp`）：
...
- `veriagent/util/functions.py::create_verify_mcps` 将工具序列转换为 FastMCP 工具并启动 uvicorn（`mcp.streamable_http_app()`）。
...
# 或使用完整类路径：
# export EX_TOOLS='["veriagent.tools.extool.SqThink","veriagent.tools.human.HumanHelp"]'
```

3. 启动后本地对话与 MCP Server 中都会出现这些工具。短名需要在 `veriagent/tools/__init__.py` 导出；否则请使用完整模块路径。
...
- 短名无效：未在 `veriagent/tools/__init__.py` 导出；改用全路径或补导出。
...
VeriAgent 内置了丰富的通用工具（如 `ReadTextFile`、`EditTextFile`、`SearchText` 等），但对于特定领域的任务，我们需要开发自定义工具。
...
1. **继承 UCTool 基类**：所有工具都继承自 `veriagent.tools.uctool.UCTool`
...
from pydantic import BaseModel, Field
from veriagent.tools.uctool import UCTool
from veriagent.tools.fileops import BaseReadWrite

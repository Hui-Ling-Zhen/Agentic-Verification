...
批处理模式（Batch Run）允许 VeriAgent 在无人值守的情况下自动完成一系列验证任务。

## 核心特性

### 1. 自动退出机制

VeriAgent 正常运行时，完成任务后会保持运行状态以便查看结果。批处理模式通过 `--exit-on-completion` 参数实现任务完成后自动退出，便于串联执行多个任务。
...
API 模式下的批处理最为简单，通过 `--exit-on-completion` 参数让 VeriAgent 在完成任务后自动退出。
...
for dut in "${DUTS[@]}"; do
    ...
    if veriagent output/ $dut -s -l --exit-on-completion --no-embed-tools; then
        echo "✓ $dut 验证成功"
    else
        echo "✗ $dut 验证失败"
        FAILED+=("$dut")
...
1. **VeriAgent MCP Server**：以 `--exit-on-completion` 参数启动
2. **Code Agent Hooks**：在 LLM 停止时触发自动继续
3. **自动推进**：通过 tmux 发送命令实现无人值守

#### Hook 消息机制

VeriAgent 提供了 `--hook-message` 参数，用于获取配置中定义的提示词：

```bash
# 查看继续提示词
veriagent --hook-message continue

# 查看完成提示词
veriagent --hook-message quit

# 从配置文件读取（格式：[config.yaml::]key[|stop_key]）
veriagent --hook-message config.yaml::continue_key|complete_key
```

工作机制：

- 检查 VeriAgent 状态（通过 `.veriagent/veriagent_info.json`）
		...
		"Stop": [
			{
				"hooks": [
					{
						"type": "command",
						"command": "M=`veriagent --hook-message 'continue|quit'` && (tmux send-keys $M; sleep 1; tmux send-keys Enter)",
						"timeout": 3
					}
				]
			}
...
- **command 执行**：
  - `veriagent --hook-message 'continue|quit'`：获取提示词
...
# 1. 启动 VeriAgent MCP Server
tmux new-session -d -s veriagent_$DUT
tmux send-keys -t veriagent_$DUT "veriagent output/ $DUT --mcp-server-port $PORT -eoc --tui --no-embed-tools" C-m
...
iflow_once:
	@while [ -f "../../output/.veriagent/veriagent_info.json" ]; do \
		sleep 5; \
	done
	@while [ ! -f "../../output/Guide_Doc/dut_fixture.md" ]; do \
		sleep 5; \
	...
	(sleep 10; tmux send-keys `veriagent --hook-message cagent_init`; sleep 1; tmux send-keys Enter)&
...
1. **左侧窗格**：依次启动 VeriAgent MCP Server（Adder → Mux → FSM）
...
```json
{
	"hooks": {
		"onStop": {
			"command": "veriagent --hook-message continue",
			"type": "shell"
		}
	}
}
...
hooks:
  stop:
    - type: command
      command: "M=$(veriagent --hook-message continue) && echo $M"
```
...
VeriAgent 内置了默认的继续提示词，可以通过 `--hook-message` 查看：
...
batch_prompts:
  ...
  complete_key: |
    所有验证任务已完成，可以退出了。感谢使用 VeriAgent！
...
```bash
# 从配置文件读取
veriagent --hook-message config.yaml::batch_prompts.continue_key

# 指定继续和退出的键
veriagent --hook-message "batch_prompts.continue_key|batch_prompts.complete_key"
```
...
# 使用环境变量
veriagent --hook-message "\$CONTINUE_MSG|\$COMPLETE_MSG"
```
...
**现象**：使用 `--exit-on-completion` 后，VeriAgent 仍未退出
...
```bash
# 查看 VeriAgent 状态
cat output/.veriagent/veriagent_info.json

# 检查是否有残留进程
ps aux | grep veriagent
...
2. 验证 Hook 命令
   ...
   ```bash
   # 手动测试
   M=$(veriagent --hook-message continue) && echo $M
   ```
...
```bash
# 添加超时和重试
timeout 7200 veriagent output/ DUT -s -l -eoc --no-embed-tools || {
    echo "任务超时或失败，等待 30 秒后重试..."
    sleep 30
    veriagent output/ DUT -s -l -eoc --no-embed-tools
}
```
...
for dut in Adder Mux FSM; do
    ...
    # 在独立目录中执行
    (cd $WORK_DIR && veriagent ../output/ $dut -s -l -eoc --no-embed-tools)
...
- [多实例并发执行](./01_multirun.md) - 同时运行多个 VeriAgent 实例
- [API 直接使用模式](../02_usage/01_direct.md) - VeriAgent 基础使用方法

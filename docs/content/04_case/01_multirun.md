...
在实际验证工作中，经常需要同时对多个 DUT 进行并发验证以提升效率。VeriAgent 支持在同一节点上运行多个实例，实现并发验证。本文档介绍如何在不同模式下实现多 VeriAgent 并发执行。

## 实现方式

VeriAgent 支持两种并发执行方式：
...
在 API 模式下，不同 workspace 下的 VeriAgent 实例是完全独立的，因此可以在多个终端窗口中同时运行，互不干扰。

### 基本用法

#### 准备工作

在运行 VeriAgent 之前，需要先准备好 DUT 文件和工作目录。如果你克隆了 VeriAgent 仓库，Makefile 会自动完成这些初始化工作（`init_Adder` 目标）：

1. 创建 RTL 目录并复制设计文件（.v/.sv/.scala 等）
2. 使用 picker 工具生成波形配置文件
3. 复制相关文档和环境文件

如果你是通过 pip 安装的 veriagent，需要手动准备工作目录：
...
在不同工作区的 `config.yaml` 中配置好模型、API_KEY 和 API_BASE 之后，就可以启动 VeriAgent 了。
**注：`config.yaml` 文件可从 VeriAgent 官方仓库根目录获取。**

#### 多终端窗口方式

准备好工作目录后，最简单的方式是在不同终端窗口中分别启动 VeriAgent：

```bash
# 终端窗口 1
cd workspace_A
veriagent output/ Adder --config config.yaml -s -hm --tui -l

# 终端窗口 2
cd workspace_B
veriagent output/ Mux --config config.yaml -s -hm --tui -l
```
...
tmux send-keys -t my_multi_api_session:0.0 "veriagent output/A/ Adder --config config.yaml -s -hm --tui -l --no-embed-tools" C-m

# 水平分割窗口
tmux split-window -h -t my_multi_api_session:0.0

# 在第二个窗格启动 Mux 验证（延迟 10 秒避免同时启动冲突，如果使用了虚拟环境，需要在tmux窗口里手动激活）
tmux send-keys -t my_multi_api_session:0.1 "sleep 10 && veriagent output/B/ Mux --config config.yaml -s -hm --tui -l --no-embed-tools" C-m
...
如果你克隆了 VeriAgent 仓库，`Makefile` 中的 `test_Adder` 目标会自动完成上述初始化步骤（通过 `init_Adder` 依赖）：
...
1. 为每个 VeriAgent 实例分配一个独立的可用端口
2. 使用该端口启动 VeriAgent MCP Server
3. 配置 Code Agent 连接到对应端口
4. 多个 "VeriAgent + Code Agent" 组合可以并发运行
...
```bash
# 使用端口 5001
veriagent output/Adder_5001/ Adder -s -hm --tui \
...
推荐使用 tmux 同时管理 VeriAgent Server 和 Code Agent：

```bash
# 创建 tmux 会话
tmux new-session -d -s mcp_5001

# 在第一个窗格启动 VeriAgent MCP Server
tmux send-keys -t mcp_5001:0.0 "veriagent output/Adder_5001/ Adder --mcp-server-port 5001 -hm --tui --mcp-server-no-file-tools --no-embed-tools" C-m
...
**现象**：启动 VeriAgent MCP Server 时提示端口已被占用
...
1. 确认 VeriAgent MCP Server 已正常启动

...
3. 准备 DUT（待测模块）
   - 创建目录：在`{工作区}`目录下创建`Adder`目录。(`{工作区}`是指当前运行`veriagent`命令的地方，其他的的目录都以`{工作区}`为根目录)
     - `mkdir -p Adder`
...
6. 配置 Qwen Code CLI
   ...
   - `unitytest`：服务器标识，用于区分不同的 MCP 服务器，此处为 VeriAgent 提供的 MCP 服务器
...
7. 启动 MCP Server<a id="启动-mcp-server"></a>
   ...
   ```bash
   veriagent output/ Adder -s -hm --tui --mcp-server-no-file-tools --no-embed-tools
   ```
...
8. 启动 Qwen Code
   - **另开一个终端**，在`VeriAgent/output`目录输入`qwen`启动 Qwen Code，看见 >QWEN 图就表示启动成功，如“图 2”所示。 ![qwen启动界面](./qwen.png)
...
- Guide_Doc：这些文件是“规范/示例/模板型”的参考文档，启动时会从`veriagent/lang/zh/doc/Guide_Doc`复制到工作区的 `Guide_Doc/`（当前以 output 作为 workspace 时即 `output/Guide_Doc/`）。它们不会被直接执行，供人和 AI 作为编写 unity_test 文档与测试的范式与规范，并被语义检索工具读取，在 VeriAgent 初始化时复制过来。
...
- 启动 `veriagent`（含 `--mcp-server`/`--mcp-server-no-file-tools`），在 TUI/MCP 下协作

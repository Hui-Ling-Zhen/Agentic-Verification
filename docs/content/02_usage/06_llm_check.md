...
在 VeriAgent 的自动化验证流程中，Checker 负责判断每个验证阶段（Stage）是否完成。传统的 Checker 通常基于规则或简单的模式匹配，虽然执行速度快，但存在以下局限：
...
为了解决上述问题，VeriAgent 引入了 **LLM Check & Refinement** 功能，将"LLM 专家"作为二次评估和指导角色，提供更智能的验证决策：
...
LLM Check & Refinement 功能通过 `veriagent/setting.yaml` 中的 `vmanager.llm_suggestion` 部分进行配置，支持两种配置方式：
...
在 `veriagent/setting.yaml` 中添加以下配置：
...
vmanager:
  llm_suggestion:
    check_fail_refinement:
      ...
      clss: veriagent.stage.llm_suggestion.OpenAILLMFailSuggestion  # 实现类
      args:
...
vmanager:
  llm_suggestion:
    check_pass_refinement:
      ...
      clss: veriagent.stage.llm_suggestion.OpenAILLMPassSuggestion  # 实现类，默认已配置
      args:
...
- **基类定义**：`veriagent/stage/llm_suggestion.py` - `BaseLLMSuggestion` 基类和工厂函数
- **LangChain 实现**：`veriagent/stage/langchain_suggestion.py` - 两种建议类的具体实现
- **集成管理**：`veriagent/stage/vmanager.py` - `StageManager` 类中的集成逻辑
- **阶段管理**：`veriagent/stage/vstage.py` - `VerifyStage` 类中的状态管理
...
- **核心实现**：
  - `veriagent/stage/llm_suggestion.py` - 基类定义
  - `veriagent/stage/langchain_suggestion.py` - LangChain 实现
  - `veriagent/stage/vmanager.py` - 集成管理
  - `veriagent/stage/vstage.py` - 阶段管理
- **配置文件**：`veriagent/setting.yaml`

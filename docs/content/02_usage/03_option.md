...
```bash
veriagent <workspace> <dut_name> {参数与选项}
```
...
```bash
# 仅 Web 访问（无本地交互）
veriagent --web-console
veriagent --web-console 0.0.0.0:8000:mysecret

# 同时提供 Web 和本地交互
veriagent ./output Adder --web-terminal
veriagent ./output Adder --web-terminal '0.0.0.0:8818 mysecret'
```
...
| --upgrade     |      | [pip_args]  | 否     | 从 GitHub main 分支升级 VeriAgent，可选传递 pip 额外参数  |
...
python3 veriagent.py ./output Adder \
  ...
  \
  --log \
  --log-file ./output/veriagent.log \
  --msg-file ./output/veriagent.msg \
  \
...
- 日志
  - --log：开启日志文件
  - --log-file ./output/veriagent.log：指定日志输出文件为`./output/veriagent.log`
  - --msg-file ./output/veriagent.msg：指定消息日志文件为`./output/veriagent.msg`
- MCP Server
...
- 版本与检查
  ...
  - --upgrade：升级 VeriAgent

## 环境变量说明

VeriAgent 支持通过环境变量配置各类参数，环境变量优先级高于配置文件设置。

...
VeriAgent 的配置文件支持 Bash 风格的环境变量占位：`$(VAR: default)`。加载时会用当前环境变量 `VAR` 的值替换；若未设置，则使用 `default`。

- 例如在内置配置 `veriagent/setting.yaml` 中：
  ...
  - 也支持其他提供商：`model_type` 可选 `openai`、`anthropic`、`google_genai`（详见 `veriagent/setting.yaml`）。
...
然后按前述命令启动 VeriAgent 即可。若要长期生效，可将上述 export 追加到你的默认 shell 启动文件（例如 bash: `~/.bashrc`，zsh: `~/.zshrc`，fish: `~/.config/fish/config.fish`），保存后重新打开终端或手动加载。
...
- 第一步和 MCP 模式相同，准备 RTL 和 SPEC。推荐路径见 [`examples/README.md`](/examples/README.md)（如 `examples/01-baseline/adder`），或通过 `make init_Adder DUT_SRC_DIR=...` 指定源目录。
- 第二步开始就不同了，打包 RTL，将文档放入工作目录并启动 VeriAgent TUI：`make test_{dut}`，`{dut}`为对应的模块。若使用 `Adder`，命令为 `make test_Adder`（可在 `Makefile` 查看全部目标）。该命令会：
  - 将 `examples/{dut}` 下文件拷贝到 `output/{dut}`（含 .v/.sv/.md/.py 等）
  - 执行 `python3 veriagent.py output/ {dut} --config config.yaml -s -hm --tui -l`
  - 启动带 TUI 的 VeriAgent，并自动进入任务循环（loop）
...
- 未安装命令时（项目内运行）：
  - `python3 veriagent.py output/ Adder --config config.yaml -s -hm --tui -l`
- 安装为命令后：
  - `veriagent output/ Adder --config config.yaml -s -hm --tui -l`

参数对齐 `veriagent/cli.py`：

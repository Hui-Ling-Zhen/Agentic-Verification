...
技能（Skill）是对一类`可复用任务方法`的封装。它把完成某种任务所需的说明、步骤、约束、脚本和辅助资料组织到一个目录中，供 VeriAgent 在合适的时候读取和使用。
...
- VeriAgent 通常先通过 `ListSkill` 了解当前可用技能，再通过 `ReadTextFile` 读取某个技能的 `SKILL.md`
...
- 这套机制当前主要面向 `veriagent/stage/vstage.py` 中的 `VerifyStage`
...
- 启用技能机制
- 将 example 对应目录下的技能（或 `--extra-skill-path`）拷贝到当前工作区的 `.veriagent/skills/` 目录下
...
含义：
- 额外拷贝指定路径下的技能到工作区 `.veriagent/skills/` 目录（优先级最高）
- 仅当设置--use-skill 参数时才能添加该参数，否则报错

技能源目录默认位于各 example 故事线下，例如：

```text
examples/01-baseline/skills/unitytest/
examples/05-formal/skills/formal/
```

`make init_<DUT>` 会把 `skills/` 复制到 workspace 根目录；也可在单个 DUT 目录下放置 `skills/` 做覆盖。

## 技能在工作区中的位置

启用后，技能会被复制到工作区下的`.veriagent/skills/`目录中,后续 `ListSkill`、`ReadTextFile`、`RunSkillScript` 看到的技能，都是以工作区中的这份副本为准。
...
- VeriAgent 在完成阶段前，不仅要使用该技能，还要通过技能使用记录校验

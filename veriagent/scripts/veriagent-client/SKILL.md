---
name: veriagent-client
description: Control a running VeriAgent CMD API server from an AI agent or shell. Use when the user wants to connect to VeriAgent, run CMD/PDB commands, inspect status, console, mission, tasks, tools, workspace files, interrupt execution, or call raw CMD API endpoints.
---

# VeriAgent Client

Use the bundled helper to communicate with a running VeriAgent
...
2. Prefer `scripts/veriagent_client.py`.
...
- Legacy state files `.uclient/veriagent-client.json` and `.uclient/veriagent.json` are
  still read for compatibility.
- Override state with `VERIAGENT_CLIENT_STATE=/path/to/state.json`.
  `VERIAGENT_CLIENT_STATE` is accepted only as a legacy override.
...
- Interrupt VeriAgent: `python3 <helper> interrupt`

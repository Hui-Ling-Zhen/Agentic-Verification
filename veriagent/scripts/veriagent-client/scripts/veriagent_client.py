#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Dependency-free helper for controlling a running VeriAgent CMD API server."""
...
DEFAULT_SOCK_PATH = "/tmp/veriagent_cmd_8765.sock"
LEGACY_SOCK_PATH = "/tmp/veriagent_cmd.sock"
STATE_ENV = "VERIAGENT_CLIENT_STATE"
LEGACY_STATE_ENV = "VERIAGENT_CLIENT_STATE"
STATE_DIRNAME = ".agents"
LEGACY_STATE_DIRNAME = ".uclient"
STATE_FILENAME = "veriagent-client.json"
LEGACY_STATE_FILENAME = "veriagent.json"


class VeriAgentClientError(RuntimeError):
    pass


class ApiError(VeriAgentClientError):
...
def default_targets() -> List[Dict[str, Any]]:
    ...
    for path in sorted(
        glob.glob("/tmp/veriagent_cmd_*.sock"),
...
def load_state(path: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        raise VeriAgentClientError(f"Failed to read state file {path}: {exc}") from exc
    if not isinstance(data, dict) or "target" not in data:
        raise VeriAgentClientError(f"Invalid state file {path}: missing target")
    return data
...
def auth_header(password: str) -> Dict[str, str]:
    if not password:
        return {}
    token = base64.b64encode(f"veriagent:{password}".encode("utf-8")).decode("ascii")
...
def request_json(
    ...
    try:
        ...
        raise VeriAgentClientError(f"{target['display']}: {exc}") from exc
    finally:
        conn.close()
...
def pick_connection(
    ...
    if use_state:
        ...
        if state:
            ...
            if not isinstance(saved_target, dict):
                raise VeriAgentClientError("Saved state has an invalid target")
    ...
    raise VeriAgentClientError(
        "No VeriAgent CMD API connection found. Tried:\n  "
        + "\n  ".join(errors)
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    for target in targets:
        try:
            ...
            print_kv(
                "Connected to VeriAgent CMD API",
                [
    ...
    print("Failed to connect to VeriAgent CMD API.", file=sys.stderr)
    print("Tried:", file=sys.stderr)
    for item in errors:
        print(f"  {item}", file=sys.stderr)
    print(
        "Hint: run `cmd_api_start` inside VeriAgent, or pass `unix:///path/to.sock` "
        "or `http://host:port`.",
        file=sys.stderr,
    )
    return 1
...
def cmd_cmd(args: argparse.Namespace) -> int:
    ...
    if not command:
        raise VeriAgentClientError("cmd requires a PDB command, for example: cmd status")
...
def cmd_disconnect(args: argparse.Namespace) -> int:
    ...
    if removed:
        print("Removed VeriAgent client connection state:")
        for path in removed:
            print(f"  {path}")
    else:
        checked = ", ".join(str(path) for path in state_candidates())
        print(f"No VeriAgent client connection state found. Checked: {checked}")
    return 0
...
def cmd_raw(args: argparse.Namespace) -> int:
    ...
    if args.body:
        try:
            ...
            raise VeriAgentClientError(f"raw body must be valid JSON: {exc}") from exc
    result = request_json(
        target,
        args.method,
        args.path,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="veriagent_client.py",
        description="Control a running VeriAgent CMD API server from an agent runtime or shell.",
    )
    subparsers = parser.add_subparsers(dest="subcommand", required=True)

    init_parser = subparsers.add_parser("init", help="Connect and save a VeriAgent CMD API target")
    ...
    console_parser = subparsers.add_parser("console", help="Show captured VeriAgent console output")
    ...
    interrupt_parser = subparsers.add_parser("interrupt", help="Send Ctrl-C interrupt to VeriAgent")
...
LOCAL_HELP = """VeriAgent client commands
...
Core:
  cmd <pdb-command> [args...]
      Enqueue a VeriAgent PDB command.
...
Connection targets:
  http://127.0.0.1:8765
  127.0.0.1:8765
  unix:///tmp/veriagent_cmd_8765.sock
  /tmp/veriagent_cmd_8765.sock

State:
  Default state file: .agents/veriagent-client.json in the current workspace.
  Legacy .uclient/veriagent-client.json and .uclient/veriagent.json are still read
  for compatibility when present.
  Override with VERIAGENT_CLIENT_STATE=/path/to/state.json.
  VERIAGENT_CLIENT_STATE is accepted as a legacy override.
"""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    except VeriAgentClientError as exc:

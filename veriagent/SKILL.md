---
name: veriagent
description: VeriAgent是基于大语言模型的自动化任务执行AI代理，支持通用工作流配置和执行。本技能提供配置文件编写规范、自定义Checker开发指南、--emulate-config配置校验工具使用方法，帮助用户快速创建、验证和运行各类任务工作流。
---
...
VeriAgent是一个基于大语言模型的自动化任务执行AI代理，支持通用工作流配置和执行。本技能提供完整的VeriAgent配置、Checker开发和任务流程指导。
...
VeriAgent使用YAML格式的配置文件定义任务、工作流和检查规则。配置文件通常包含以下核心部分：
...
Checker是VeriAgent的质量保证组件，在每个阶段完成后自动运行，验证输出是否符合预期。

### 2.1 内置通用Checker

VeriAgent内置了以下通用Checker，可直接在配置中使用：
...
所有自定义Checker都需要继承`veriagent.checkers.base.Checker`基类，并实现`do_check`方法：
...
VeriAgent提供`--emulate-config`参数(需要通过`--config`指定配置文件)，用于在正式运行前验证配置文件的正确性，避免运行过程中出现配置错误。

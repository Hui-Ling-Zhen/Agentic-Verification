# -*- coding: utf-8 -*-
"""Web-based terminal server for VeriAgent.
...
log = logging.getLogger("veriagent.terminal")
...
def _resolve_web_console_bind(spec: str) -> tuple[str, int, str]:
    ...
    from veriagent.util.functions import find_available_port, is_port_free
...
def _serve_web_console(argv: Optional[List[str]] = None) -> None:
    """Serve the VeriAgent TUI in a browser via a PTY-based web terminal."""
    ...
    if requested_capture_path:
        ...
        capture = tempfile.NamedTemporaryFile(
            prefix="veriagent_web_console_", suffix=".log", delete=False
        )
...
class PdbWebTermServer:
    ...
    **Console mode**:  initialized with ``console_input_callback`` – the
    ...
    def _print_banner(self) -> None:
        """Print VeriAgent ASCII art banner."""
        from veriagent.version import banner
        ...
        if self.password:
            ...
            async def auth_mw(request: web.Request, handler: Any) -> web.StreamResponse:
                ...
                raise web.HTTPUnauthorized(
                    text="Unauthorized",
                    headers={"WWW-Authenticate": 'Basic realm="VeriAgent Terminal"'},
                )

# -*- coding: utf-8 -*-
"""
Master API server and client for VeriAgent.

This module now serves three roles:

1. Aggregate heartbeats from multiple VeriAgent instances.
2. Launch and manage new VeriAgent subprocess tasks.
...
_MASTER_SOURCE_CONTAINER_PATH = "/VeriAgent"
...
def _task_stderr_tail(task: Dict[str, Any], max_lines: int = 200) -> str:
    ...
    if _file_contains_any(web_console_log, (
        ...
        "VeriAgent encountered an error:",
...
def _normalize_cluster_master_ip_config(value: Any) -> Any:
    env_host = str(os.environ.get("VERIAGENT_LAUNCH_MASTER_IP") or os.environ.get("VERIAGENT_LAUNCH_MASTER_IP") or "").strip()
    if env_host:
        return env_host
    default_hosts = {
        "process": "127.0.0.1",
        "docker": "host.docker.internal",
        "docker_swarm": "veriagent_master",
        "k8s": "veriagent_master",
    }
...
class PdbMasterApiServer:
    ...
    def __init__(
        ...
        if sock is None:
            sock = f"/tmp/veriagent_master_{port}.sock"
        elif sock == "":
            sock = None
        ...
        self._db_dir = get_abs_path_cwd_veriagent(self.workspace, "master_db")
    ...
    def _create_compiled_workspace_archive(self, ws: Dict[str, Any], archive_stem: str = "") -> Tuple[str, str, str]:
        ...
        temp_dir = tempfile.mkdtemp(prefix="veriagent_workspace_archive_")
    ...
    def _create_workspace(self) -> Dict[str, Any]:
        ...
        ws_dir = tempfile.mkdtemp(prefix="veriagent_launch_", dir=base_root)
    ...
    def _cleanup_stale_workspaces(self) -> int:
        ...
        for entry in os.listdir(base_root):
            if not entry.startswith("veriagent_launch_"):
                continue
    ...
    def _build_veriagent_command(self, req: Dict[str, Any], prepared: Dict[str, Any], cmd_api: Dict[str, Any]) -> Tuple[List[str], Dict[str, str]]:
        ...
        if self.access_key:
            env["VERIAGENT_WORKSPACE_ARCHIVE_KEY"] = self.access_key
        if self.password:
            env["VERIAGENT_WORKSPACE_ARCHIVE_PASSWORD"] = self.password
        return argv, env
    ...
    def _launch_cluster_config(self) -> Dict[str, Any]:
        ...
        if not isinstance(cfg, dict):
            cfg = {}
        return {
            "image": str(cfg.get("image") or os.environ.get("VERIAGENT_LAUNCH_IMAGE") or os.environ.get("VERIAGENT_LAUNCH_IMAGE") or "veriagent:latest"),
            "container_command": cfg.get("container_command") or "veriagent",
    ...
    def _master_source_host_path(self) -> str:
        raw = str(os.environ.get("VERIAGENT_MASTER_SOURCE", "") or "").strip()
        if not raw:
            return ""
    ...
    def _master_source_cli_path(self) -> str:
        ...
        if not source_host_path:
            return ""
        return os.path.join(source_host_path, "veriagent", "cli.py")

    def _master_source_container_cli_path(self) -> str:
        return os.path.join(_MASTER_SOURCE_CONTAINER_PATH, "veriagent", "cli.py")

    def _container_command(self, resolved_command: List[str]) -> List[str]:
        cfg = self._launch_cluster_config()
        configured = cfg.get("container_command") or "veriagent"
        ...
        if isinstance(configured, list):
            ...
            prefix = shlex.split(str(configured)) if str(configured).strip() else ["veriagent"]
    ...
    def _k8s_manifest(
        ...
        return {
            ...
            "metadata": {
                ...
                "labels": {
                    "app": "veriagent",
                    "veriagent-task-id": task["task_id"],
                },
            },
            "spec": {
                "backoffLimit": 0,
    ...
    def _run_task_launch(self, req: Dict[str, Any]) -> Dict[str, Any]:
        ...
        resolved_command, env = self._build_veriagent_command(req, prepared, cmd_api)
    ...
    def _build_app(self):  # noqa: C901
        ...
        app = FastAPI(
            title="VeriAgent Master API",
            description="Aggregates heartbeats and manages launched VeriAgent tasks.",
            version="2.0.0",
        )
        ...
        async def _check_password(credentials: Optional[HTTPBasicCredentials] = Depends(security)):
            ...
            if credentials is None or not _secrets.compare_digest(
                ...
                raise HTTPException(
                    ...
                    headers={"WWW-Authenticate": 'Basic realm="VeriAgent Master"'},
                )
        ...
        def index():
            ...
            try:
                ...
                return HTMLResponse("<h2>VeriAgent Master API</h2><p><a href='/docs'>Swagger UI</a></p>")
...
class PdbMasterClient:
    ...
    def _build_payload(self) -> dict:
        ...
        run_time = ""
        try:
            from veriagent.util.functions import fmt_time_deta as _fmt_time_deta

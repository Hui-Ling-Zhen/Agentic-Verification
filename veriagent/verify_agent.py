...
class VerifyAgent:
    ...
    def __init__(
        ...
        self.tool_list_ext = import_and_instance_tools(
            self.cfg.get_value("ex_tools", []), veriagent.tools
        ) + import_and_instance_tools(ex_tools, veriagent.tools) + self.tool_skill

        # Export workspace path via environment variable for ext tools
        os.environ["VERIAGENT_WORKSPACE"] = self.workspace
    ...
    def render_template(self, template_cfg=None, tmp_overwrite=False):
        template_context = {
            ...
            "Email": __email__,
            "CWD": self.workspace,
            "UC_LIB_PATH": veriagent_lib_path(),
        }

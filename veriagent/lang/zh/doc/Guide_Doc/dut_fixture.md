...
def create_dut(request):
    ...
    if veriagent.is_imp_test_template():
        return veriagent.get_fake_dut(DUT{{DutClass}})
...
def dut(request):
    ...
    set_user_info("VeriAgent-{{Version}}", "{{Email}}")
...
- 在create_dut需要通过veriagent.is_imp_test_template判断当前是否是测试模板生成阶段
-  -是则需要通过veriagent.get_fake_dut返回fake dut用于提速

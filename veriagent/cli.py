...
This module provides the command line interface for VeriAgent, 
...
def _download_workspace_archive(source_url: str, archive_path: str) -> None:
    ...
    headers = {"User-Agent": f"VeriAgent/{__version__}"}
    query = parse_qs(parsed.query, keep_blank_values=True)
    access_keys = query.pop("key", [])
    if access_keys and access_keys[0]:
        headers["X-Access-Key"] = access_keys[0]
    elif os.environ.get("VERIAGENT_WORKSPACE_ARCHIVE_KEY"):
        headers["X-Access-Key"] = os.environ["VERIAGENT_WORKSPACE_ARCHIVE_KEY"]
    if os.environ.get("VERIAGENT_WORKSPACE_ARCHIVE_PASSWORD"):
        token = base64.b64encode(f":{os.environ['VERIAGENT_WORKSPACE_ARCHIVE_PASSWORD']}".encode("utf-8")).decode("ascii")
...
def _prepare_workspace_archive_source(workspace: str, dut_name: str, workspace_base: str) -> str:
    ...
    base_dir = os.path.abspath(os.path.expanduser(workspace_base or "/tmp/veriagent_workspace_base"))
    extract_dir = os.path.join(base_dir, archive_name)
    marker_path = os.path.join(extract_dir, ".veriagent_archive_extract")
    try:
    ...
    if os.path.exists(extract_dir):
        if os.path.isfile(marker_path):
            ...
            raise WorkspaceArchiveError(
                f"Extraction target already exists and was not created by VeriAgent: {extract_dir}. "
                "Remove it or choose another --workspace-base."
            )
    try:
        os.makedirs(extract_dir, exist_ok=False)
        with tempfile.TemporaryDirectory(prefix="veriagent_workspace_archive_") as tmp_dir:
...
class HookMessageAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        import veriagent.util.log as log
...
def get_args() -> argparse.Namespace:
    ...
    prog_name = "veriagent"
    if sys.argv[0].endswith("veriagent.py"):
        prog_name = "veriagent.py"
    
    parser = argparse.ArgumentParser(
        description="VeriAgent - Agentic Verification CLI",
        prog=prog_name,
        epilog="For more information, visit: https://github.com/Hui-Ling-Zhen/Agentic-Verification"
    )
    ...
    parser.add_argument(
        "--workspace-base",
        type=str,
        default="/tmp/veriagent_workspace_base",
        help=(
            ...
            "<workspace-base>/<archive-name>/workspace. Defaults to /tmp/veriagent_workspace_base "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "If no path is provided, default to /tmp/veriagent_master."
        )
    )
    ...
    parser.add_argument("--enable-context-manage-tools", action="store_true", default=False,
                        help="Enable context management tools. This is useful when you run VeriAgent in the API mode.")
    ...
    parser.add_argument(
        "--version", 
        action="version", 
        version="VeriAgent Version: " + __version__,
    )
    ...
    parser.add_argument(
        ...
        help="Upgrade VeriAgent to the latest version from GitHub main branch. "
...
def upgrade(extra_pip_args: str = "") -> None:
    ...
    info(f"Upgrading VeriAgent from GitHub main branch using Python {sys.version.split()[0]}...")
    ...
    for url in ["https://github.com/XS-MLVP",
        ...
        try:
            ...
            source_url = f'git+https://github.com/Hui-Ling-Zhen/Agentic-Verification@main'
...
def do_check() -> None:
    ...
    default_user_config_path = os.path.join(os.path.expanduser("~"), ".veriagent/setting.yaml")
    echo_g("VeriAgent version: " + __version__)
...
def run() -> None:
    """Main entry point for VeriAgent CLI."""
    args = get_args()
    ...
    if args.web_console is not None and \
       ...
       args.web_console_session_port is None:
        from veriagent.server.api_terminal import _serve_web_console
        try:
    ...
    if getattr(args, 'as_master', None) is not None and args.workspace is None:
        if getattr(args, 'as_master_persist', None) is not None:
            ...
            temp_dir = tempfile.TemporaryDirectory(prefix="veriagent_master_")
    ...
    if args.emulate_config:
        if args.config is None:
            ...
            sys.exit(1)
        temp_dir = tempfile.TemporaryDirectory(prefix="veriagent_emulate_")
        args.workspace = temp_dir.name
        args.dut = "DUT_TEST"
    ...
    if args.workspace is None or args.dut is None:
        ...
        _p = _argparse.ArgumentParser(prog="veriagent")
...
def main() -> None:
    ...
    try:
        ...
        info("\nVeriAgent interrupted by user.")
        sys.exit(1)
    except Exception as e:
        info(f"VeriAgent encountered an error: {e}")
        ...
        if capture_path:
            try:
                ...
                with open(capture_path, "a", encoding="utf-8", errors="replace") as fh:
                    fh.write(f"VeriAgent encountered an error: {e}\n")
                    fh.write(formatted)

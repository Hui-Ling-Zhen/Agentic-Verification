...
def get_abs_path_cwd_veriagent(workspace, path):
    """
    ...
    """
    veriagent_path = os.path.abspath(workspace + os.sep + ".veriagent")
    if not os.path.exists(veriagent_path):
        os.makedirs(veriagent_path)
    return os.path.abspath(veriagent_path + os.sep + path)


def save_veriagent_info(workspace, info: dict):
    """
    Save VeriAgent information to a JSON file in the workspace.
    :param workspace: The workspace directory where the file will be saved.
    :param info: The VeriAgent information to be saved.
    """
    assert os.path.exists(workspace), f"Workspace {workspace} does not exist."
    info_path = get_abs_path_cwd_veriagent(workspace, "veriagent_info.json")
    save_json_file(info_path, info)


def load_veriagent_info(workspace) -> dict:
    """
    Load VeriAgent information from a JSON file in the workspace.
    :param workspace: The workspace directory where the file is located.
    :return: The loaded VeriAgent information.
    """
...
def create_verify_mcps(mcp_tools: list, host: str, port: int, logger=None):
    ...
    try:
        ...
        from veriagent.tools.uctool import to_fastmcp
        from veriagent.util.log import info
        fastmcp_tools = []
...
def is_veriagent_complete(workspace=".", need_agent_exit=False):
    """Check VeriAgent is complete from file"""
    status_data = load_veriagent_info(workspace)
...
def get_veriagent_hook_msg(
    ...
    """Get VeriAgent hook message from file"""
    status_data = load_veriagent_info(workspace)
    if not status_data:
        return msg_init
...
def resolve_skill_source_paths(cfg, workspace, dut_name=None, root_dir=None):
    """Resolve skill source directories for the current task.

    Skills live under each example directory (see examples/*/skills/) and are
    copied into the workspace during init. Resolution order (first wins for copy,
    all existing paths are merged):

    1. --extra-skill-path (explicit override)
    2. {workspace}/{DUT}/skills/  (per-DUT customization)
    3. {workspace}/skills/        (storyline skills copied by Makefile init)
    """
    paths = []
    seen = set()
    ...
    if cfg.skill.extra_skill_path:
        _add(cfg.skill.extra_skill_path)
    if dut_name:
        _add(os.path.join(workspace, dut_name, "skills"))
    _add(os.path.join(workspace, "skills"))
    return paths


def find_skill_script(workspace, relative_path, root_dir=None):
    """Locate a skill script under workspace skill roots.

    Args:
        workspace: Task workspace directory
        relative_path: Path relative to skills root, e.g.
            ``formal/sva-gen/scripts/generate_formal_env.py``
        root_dir: Optional veriagent package root for legacy fallback
    """
    rel = relative_path.replace("\\", "/").lstrip("/")
    candidates = []
    skill_root = get_workspace_skill_root(workspace)
    candidates.append(os.path.join(skill_root, rel))
    candidates.append(os.path.join(workspace, "skills", rel))
    if root_dir:
        candidates.append(os.path.join(root_dir, "lang", "zh", "skills", rel))
...
def copy_skill_files(cfg, workspace, root_dir, dut_name=None):
    """Copy skill files from example/workspace paths into .veriagent/skills."""
    dst_path = get_workspace_skill_root(workspace)
    src_paths = resolve_skill_source_paths(cfg, workspace, dut_name=dut_name, root_dir=root_dir)
    if not src_paths:
        warning(
            "No skill source directory found. Expected skills under "
            f"{os.path.join(workspace, 'skills')} or --extra-skill-path."
        )
        return
    for src_path in src_paths:
        try:
            copied_skills = copytree_incremental(
                src_path,
                dst_path,
                skip_existing=True,
                enable_skill_list=cfg.skill.enable_skill_list,
                disable_skill_list=cfg.skill.disable_skill_list,
            )
            if copied_skills:
                info(f"Copy {len(copied_skills)} skill(s) from '{src_path}'")
        except Exception as e:
            warning(f"Failed to copy skills from '{src_path}': {e}")
...
def get_workspace_skill_root(workspace):
    return get_abs_path_cwd_veriagent(workspace, "skills")

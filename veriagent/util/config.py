...
from .functions import render_template, dump_as_json, replace_bash_var, get_abs_path_cwd_veriagent
...
class VeriAgentConfigLoader(yaml.SafeLoader):
...
VeriAgentConfigLoader.add_implicit_resolver('!negated_bool', _NEGATED_BOOL_PATTERN, ['-', 'n', 'N'])
VeriAgentConfigLoader.add_constructor('!negated_bool', _construct_negated_bool)

class Config:
    """Configuration class for VeriAgent settings."""
...
def load_yaml_with_env_vars(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        return yaml.load(rendered_content, Loader=VeriAgentConfigLoader)
...
def get_config(config_file=None, cfg_override=None, workspace=None):
    ...
    user_config_file = os.path.abspath(os.path.join(user_home, '.veriagent/setting.yaml'))
    if os.path.isfile(user_config_file):
        ...
        with open(user_config_file, 'w') as f:
            f.write("# VeriAgent user configuration file\n")
        loaded_configs.append(user_config_file)

    # 3. load minimal runtime lang scaffold (empty workflow; mission/stages from --config)
    lang = cfg.get_value('lang', 'zh')
    lang_config_file = os.path.abspath(
        os.path.join(os.path.dirname(__file__), f"../lang/{lang}/config/empty.yaml")
    )
    if os.path.isfile(lang_config_file):
        cfg.merge_from(Config(load_yaml_with_env_vars(lang_config_file)))
        info(f"Load config from '{lang_config_file}' completed.")
        loaded_configs.append(lang_config_file)
    else:
        info(f"Lang scaffold config '{lang_config_file}' not found, skip.")

    # 4. load workspace config
    if workspace is not None:
        cwd_setting_file = get_abs_path_cwd_veriagent(workspace, "setting.yaml")

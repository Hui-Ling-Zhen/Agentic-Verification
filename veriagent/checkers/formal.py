...
from veriagent.util.functions import find_skill_script


# Shared utilities – single source of truth
from veriagent.lang.zh.skills.formal.lib.formal_paths import FormalPaths
from veriagent.lang.zh.skills.formal.lib.formal_tools import (
    parse_avis_log,
...
class BaseFormalChecker(Checker):
    ...
    def _ensure_tcl_executed(self, dep_paths: list, tcl_script_path: str, timeout: int = 300) -> tuple[bool, dict, bool]:
        ...
        try:
            from veriagent.util import diff_ops
...
class FormalSpecJsonChecker(BaseFormalChecker):
    ...
    def do_check(self, timeout=0, **kwargs) -> tuple[bool, object]:
        ...
        if self._check_level == "ck":
            try:
                from veriagent.lang.zh.skills.formal.lib.formal_tools import generate_spec_doc
...
class EnvironmentAnalysisChecker(BaseFormalChecker):
    ...
    def do_check(self, timeout=300, **kwargs) -> tuple:
        ...
        try:
            from veriagent.lang.zh.skills.formal.lib.formal_tools import generate_env_analysis_doc
...
class CounterexampleTestgenChecker(BaseFormalChecker):
    ...
    def do_check(self, timeout=0, **kwargs) -> tuple[bool, object]:
        ...
        if rtl_bugs and not os.path.exists(test_path):
            try:
                import subprocess
                cex_script = find_skill_script(
                    self.workspace,
                    "formal/cex-gen/scripts/init_test_file.py",
                    root_dir=os.path.join(os.path.dirname(os.path.abspath(__file__)), ".."),
                )
                if cex_script:
                    result = subprocess.run(
...
class BugReportConsistencyChecker(BaseFormalChecker):
    ...
    def do_check(self, timeout=0, **kwargs) -> tuple[bool, object]:
        ...
        try:
            from veriagent.lang.zh.skills.formal.lib.formal_tools import generate_bug_report_doc

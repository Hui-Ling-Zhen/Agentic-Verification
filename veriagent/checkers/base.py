# -*- coding: utf-8 -*-
"""Base checker class for VeriAgent verification checkers."""

import os
import sys
from typing import Tuple
from veriagent.util.config import Config
from veriagent.util.functions import render_template, rm_workspace_prefix, fill_template
import veriagent.util.functions as fc
from veriagent.util.log import info, error, warning
import time
import traceback
import hashlib
...
class UnityChipBatchTask:
    ...
    * ``savepoint_file()`` writes the four task lists to a JSON checkpoint under
      the VeriAgent working directory so runs can be inspected and resumed.
    """
    ...
    def on_init(self):
        ...
        self.checkpoint_file = fc.get_abs_path_cwd_veriagent(self.checker.workspace, f"batch_checkpoint_{chp_name}.json")
        self.loadpoint_file()

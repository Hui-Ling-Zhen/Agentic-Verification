# -*- coding: utf-8 -*-
"""Unity test checker for VeriAgent verification."""
...
from veriagent.tools.testops import RunUnityChipTest
import os
import glob
import traceback
import copy
...
from veriagent.checkers.base import Checker, UnityChipBatchTask
from veriagent.checkers.toffee_report import check_report, check_line_coverage
...
class UnityChipCheckerDutCreation(Checker):
    def __init__(self, target_file, **kw):
        ...
        veriagent_msg = f"You need use:\n`if veriagent.is_imp_test_template():\n" + \
                      f"    return veriagent.get_fake_dut(DUT{self.dut_name})`\n in 'create_dut' function."
        self.source_code_need = {
            ...
            "veriagent.is_imp_test_template": (veriagent_msg, None),
            "veriagent.get_fake_dut":(veriagent_msg, None)
        }
...
class UnityChipCheckerMockFixture(UnityChipCheckerBaseFixture):
    def __init__(self, target_file, min_count=1, **kw):
        ...
        self.update_dut_name(kw["cfg"])
        veriagent_msg = f"You need use:\n`def mock_dut():\n    return veriagent.get_mock_dut_from(DUT{self.dut_name})\n` in 'mock_dut' fixture."
        self.source_code_need = {
            "veriagent.get_mock_dut_from": (veriagent_msg, None)
        }

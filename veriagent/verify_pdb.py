# -*- coding: utf-8 -*-
"""Specialized PDB debugger for VeriAgent verification."""
...
import os
import sys
from veriagent.util.log import echo_g, echo_y, echo_r, echo, info, message, set_console_sync_handler
from veriagent.util.functions import dump_as_json, get_func_arg_list, fmt_time_deta, fmt_time_stamp, list_files_by_mtime, yam_str, is_port_free
import time
import signal
import threading
import traceback
from veriagent.util.log import L_GREEN, L_YELLOW, L_RED, RESET, L_BLUE
import readline
import random
from collections import OrderedDict
from typing import TYPE_CHECKING

from veriagent.tui.utils import PersistentConsoleMirror

if TYPE_CHECKING:
    from veriagent.tui.widgets.console import ConsoleWidgetState
    from veriagent.tui.widgets.messages_panel import MessagesPanelState
...
class VerifyPDB(Pdb):
    ...
    def __init__(self, agent, prompt = "(UnityChip) ", init_cmd=None,
        ...
        self.history_file = os.path.expanduser("~/.veriagent/pdb_cmd_history")
        try:
    ...
    def _new_console_entry(self, kind: str, payload: str):
        from veriagent.tui.widgets.console import ConsoleEntry
    ...
    def do_tui(self, arg):
        ...
        if self._in_tui:
            ...
            return
        from veriagent.tui import enter_tui
    ...
    # sock=None passed to PdbCmdApiServer means "auto-generate /tmp/veriagent_cmd_{port}.sock"
    ...
    def do_cmd_api_start(self, arg):
        ...
        Options:
          --sock <path>   Unix socket path  (default: /tmp/veriagent_cmd_{port}.sock)
        ...
        if self._cmd_api_server is not None and self._cmd_api_server.is_running:
            ...
            return
        from veriagent.server import PdbCmdApiServer
        host = self.agent.cfg.get_value("cmd_api.host", "127.0.0.1")
        port = self.agent.cfg.get_value("cmd_api.port", 8765)
        port_specified = False
        sock = None   # None → server auto-generates /tmp/veriagent_cmd_{port}.sock
    ...
    def do_terminal_api_start(self, arg):
        ...
        Maps the current VeriAgent console I/O (PDB command line or TUI) to a
        ...
        if self._in_tui:
            ...
            return
        from veriagent.server.api_terminal import PdbWebTermServer
    ...
    # sock=None passed to PdbMasterApiServer means "auto-generate /tmp/veriagent_master_{port}.sock"
    # sock=""  means "disable unix socket"

    def do_master_api_start(self, arg):
        """
        Start the Master API server (FastAPI).  Acts as a central aggregator
        that collects heartbeats from multiple VeriAgent instances.

        Usage: master_api_start [options] [host [port]]

        Options:
          --sock <path>       Unix socket path  (default: /tmp/veriagent_master_{port}.sock)
          --sock none         Disable Unix socket listener
          --no-tcp            Disable TCP listener
        ...
        if self._master_api_server is not None and self._master_api_server.is_running:
            ...
            return
        from veriagent.server import PdbMasterApiServer
        host = self.agent.cfg.get_value("master_api.host", "0.0.0.0")
        port = self.agent.cfg.get_value("master_api.port", 8800)
        port_specified = False
        sock = None   # None → server auto-generates /tmp/veriagent_master_{port}.sock
        tcp = True
    ...
    def do_connect_master_to(self, arg):
        ...
        """
        from veriagent.server import PdbMasterClient
        parts = arg.strip().split()
        if not parts:
    ...
    def api_load_toffee_report(self, path, workspace):
        ...
        """
        from veriagent.util.functions import load_toffee_report

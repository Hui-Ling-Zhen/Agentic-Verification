...
from veriagent.checkers import UnityChipCheckerTestFree
from veriagent.stage.vstage import get_root_stage
from veriagent.tools.uctool import UCTool, EmptyArgs
from veriagent.util.functions import make_llm_tool_ret
...
class StageManager(object):
    def __init__(
            self, workspace, cfg, agent, tool_read_text, veriagent_info: dict,
            force_stage_index=0,
            force_todo=False,
            todo_panel=None,
        ...
        self.veriagent_info = veriagent_info
    ...
    def init_stage(self):
        from veriagent.stage import VerifyStage
        ...
        stages_info = self.veriagent_info.get("stages_info", {})
        ...
        self.all_completed = bool(self.veriagent_info.get("all_completed", False))
        ...
        self.time_begin = self.veriagent_info.get("time_begin", time.time())
        self.time_end = self.veriagent_info.get("time_end", None)
    ...
    def exit(self):
        ...
        if self._refresh_all_completed():
            ...
            if self.agent._exit_on_completion:
                ex_msg = " VeriAgent has quit. The MCP server is shutting down — all MCP tools will become unavailable. You need to stop Now!"
            return {
                "exit": True,

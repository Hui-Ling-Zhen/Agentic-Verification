...
from veriagent.server import api_terminal
from veriagent.server.api_terminal import PdbWebTermServer, _get_web_console_exit_code
...
def test_web_console_prints_traceback_even_when_exit_code_is_zero(monkeypatch):
    ...
    try:
        ...
        lines += [
            b"VeriAgent encountered an error: boom\n",
            b"Traceback (most recent call last):\n",
            b"  File \"demo.py\", line 1, in <module>\n",
            b"RuntimeError: boom\n",
            b"VeriAgent is exited.\n",
        ]
...
def test_cli_writes_exception_to_web_console_capture_arg():
    ...
    try:
        command = shlex.join([
            sys.executable,
            os.path.abspath(os.path.join(current_dir, "..", "veriagent", "cli.py")),
            "/tmp/no-workspace",
            "NoDut",
            "--extra-skill-path",
            "bad-skill-path",
        ...
        output = open(capture.name, "rb").read().decode("utf-8", "replace")
        assert "VeriAgent encountered an error" in output
